...
import { Bell, Lock, Eye, CreditCard, User, HelpCircle, ChevronRight, LogOut, ShieldCheck } from 'lucide-react';
...
const SettingItem = ({ icon: Icon, title, sub, hasToggle, hasArrow, to }: any) => {
  ...
  const content = (
    <div 
      className="flex items-center justify-between p-6 bg-white rounded-3xl border border-gray-100 hover:border-sage/20 transition-all cursor-pointer group w-full text-left"
      onClick={toggle}
    >
      ...
      <div className="flex items-center gap-4">
        {hasToggle && (
          <div className={`w-10 h-6 rounded-full relative transition-colors ${isOn ? 'bg-forest' : 'bg-sage/20'}`}>
            <div className={`absolute top-1 w-4 h-4 rounded-full transition-all bg-white shadow-sm ${isOn ? 'right-1' : 'left-1'}`} />
          </div>
        )}
        {hasArrow && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <SettingItem icon={HelpCircle} title="Terms & Conditions" sub="Revised March 2024" hasArrow={true} />
            </div>
          </section>
        </div>
